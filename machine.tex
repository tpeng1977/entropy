...
Table~\ref{tab:process_energy} gives, for each process, the heat and work
amounts (per unit mass). Figure~\ref{fig:energy_flow} gives an intuitive
picture of the energy flows. \textbf{Energy to maintain the temperature
difference:} If the 1--2\,$^\circ$C difference were maintained by an
auxiliary heat pump, the work required would be $w_{\mathrm{HP,ideal}} \approx
0.006\,\mathrm{kJ}/\mathrm{kg}$ (ideal) or $w_{\mathrm{HP}} \approx
0.015$--$0.03\,\mathrm{kJ}/\mathrm{kg}$ (realistic COP); see the paragraph
below and Table~\ref{tab:process_energy}. That is only about 1--6\% of
$w_{\mathrm{net}}$, so the cycle can sustain $\Delta T$ with a small fraction
of its output and still deliver surplus.

\begin{table}[htbp]
\centering
\caption{Energy balance per process (per unit mass, kJ/kg). Heat in/out and work in/out for each step; last row: optional auxiliary work to maintain the 1--2\,$^\circ$C difference.}
\label{tab:process_energy}
\begin{tabular}{@{}lcccc@{}}
\toprule
Process & $Q_{\mathrm{in}}$ & $Q_{\mathrm{out}}$ & $W_{\mathrm{in}}$ & $W_{\mathrm{out}}$ \\
\midrule
...
\midrule
Auxiliary (maintain $\Delta T$, optional) & -- & -- & 0.006--0.03 & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.9, every node/.style={font=\small},
  ...
  \node[box, minimum height=3.2cm] (env) at (0,-0.6) {Environment};
  \coordinate (e1) at ([yshift=0.9cm]env.east);
  \coordinate (e2) at ([yshift=0.3cm]env.east);
  \coordinate (e3) at ([yshift=-0.3cm]env.east);
  \coordinate (e4) at ([yshift=-0.9cm]env.east);
  \node[box] (hp) at (4.5,1.2) {HP side (2$\to$3)};
  \node[box] (pump) at (4.5,0) {Pump (1$\to$2)};
  \node[box] (exp) at (4.5,-1.2) {Expander (3$\to$4)};
  \node[box] (ch) at (4.5,-2.4) {Chamber (4$\to$1)};
  \draw[->, thick] (e1) -- (hp.west) node[lbl, pos=0.6, above=1pt] {$Q_{\mathrm{in}}{=}0.9$};
  \draw[->, thick] (e2) -- (pump.west) node[lbl, pos=0.6, above=1pt] {$W_{\mathrm{in}}{=}0.026$};
  \draw[->, thick] (exp.west) -- (e3) node[lbl, pos=0.4, above=1pt] {$W_{\mathrm{out}}{=}0.54$};
  \draw[->, thick] (ch.west) -- (e4) node[lbl, pos=0.4, below=1pt] {$Q_{\mathrm{out}}{=}0.386$};
  \node[align=center, font=\footnotesize] at (2.25,-3.5) {Net: $w_{\mathrm{net}}=0.54-0.026=0.514$; optional $W_{\mathrm{HP}}\approx0.006$--0.03 to maintain $\Delta T$.};
\end{tikzpicture}
\caption{Energy flows per cycle (per kg, kJ/kg). Heat from environment to HP side; work from environment (or from part of net output) to pump; work from expander to environment; heat rejected from chamber to environment. Optional auxiliary work to maintain the micro $\Delta T$ is 0.006--0.03\,kJ/kg (see text). This figure is distinct from Fig.~\ref{fig:cycle} (process layout).}
