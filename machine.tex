...
2.82\,\mathrm{kJ}/\mathrm{kg}$ per cycle), delivers net useful work
($w_{\mathrm{net}} = +0.687\,\mathrm{kJ}/\mathrm{kg}$), and uses only standard
...
Statistical thermodynamics states that, in the absence of targeted design, a
``miracle'' outcome (e.g.\ net entropy decrease of the environment, or net
work from a single reservoir) has vanishing probability when viewed over all
possible trajectories. That conclusion applies to situations where no
deliberate, stepwise choice of parameters and constraints is made. In the
present approach, every step and every component is a deliberate choice: working
fluid, pressures, temperatures, geometry, liquid-only compression path,
heat-exchange locations, and so on. We are not sampling at random; we are
making a long sequence of targeted choices. Out of an astronomical number of
possibilities, that sequence selects the trajectory that would otherwise be
extremely unlikely---we fix the path by design rather than waiting for it to
occur by chance. So ``the miracle is almost impossible'' refers to the regime
without such design; with repeated careful choice, the same outcome is
selected. The conventional laws were formulated for the former regime and do not
...
NIST~\cite{nistwebbook}; saturation values from NIST Chemistry WebBook, see e.g.\ \url{https://webbook.nist.gov}; all pressures absolute): $p_{\mathrm{L}} = p_{\mathrm{sat}}(24\,^\circ\mathrm{C})
= 645.8\,\mathrm{kPa}$ ($0.6458\,\mathrm{MPa}$), $p_{\mathrm{H}} = p_{\mathrm{sat}}(26\,^\circ\mathrm{C})
= 685.4\,\mathrm{kPa}$ ($0.6854\,\mathrm{MPa}$); saturated liquid enthalpy
$h_{\mathrm{f,L}} = 84.98\,\mathrm{kJ}/\mathrm{kg}$,
$h_{\mathrm{f,H}} = 87.83\,\mathrm{kJ}/\mathrm{kg}$; liquid specific volume
$v_{\mathrm{f}} = 0.0008261\,\mathrm{m}^3/\mathrm{kg}$ (24\,$^\circ$C); latent heat at 24\,$^\circ$C
$h_{\mathrm{fg}} = 178.70\,\mathrm{kJ}/\mathrm{kg}$.\footnote{NIST-aligned values (saturation, temperature table): $p_{\mathrm{sat}}(24\,^\circ\mathrm{C}) = 645.8\,\mathrm{kPa}$, $p_{\mathrm{sat}}(26\,^\circ\mathrm{C}) = 685.4\,\mathrm{kPa}$, $h_{\mathrm{f,L}} = 84.98$, $h_{\mathrm{f,H}} = 87.83\,\mathrm{kJ}/\mathrm{kg}$, $v_{\mathrm{f}} = 0.0008261\,\mathrm{m}^3/\mathrm{kg}$, $h_{\mathrm{fg}}(24\,^\circ\mathrm{C}) = 178.70\,\mathrm{kJ}/\mathrm{kg}$. Source: NIST Chemistry WebBook (R134a).} All cycle calculations
...
\midrule
1 & 0.646 & 24 & 84.98 & Saturated liquid (chamber bottom) \\
2 & 0.685 & 24 & 85.01 & Subcooled liquid (after pump) \\
3 & 0.685 & 26 & 87.83 & Saturated liquid at $p_{\mathrm{H}}$ (after $q_{\mathrm{in}}$) \\
4 & 0.646 & 24 & 87.11 & Liquid + vapour (chamber; $h_4 = h_3 - w_{\mathrm{out}}$) \\
\bottomrule
\end{tabular}
\end{table}
State 4 is two-phase; $h_4$ lies between $h_{\mathrm{f,L}}$ and
$h_{\mathrm{f,L}}+h_{\mathrm{fg}}$ and is fixed by the first law in process 3$\to$4 (adiabatic expander): $h_4 = h_3 - w_{\mathrm{out}} = 87.11\,\mathrm{kJ}/\mathrm{kg}$ (process 3$\to$4 below). The balance
$q_{\mathrm{in}} - q_{\mathrm{out}} = w_{\mathrm{net}}$ holds and is consistent with $q_{\mathrm{out}} = h_4 - h_1$ in process 4$\to$1. The state-to-state path is unambiguous.
...
Pressure $p_1 = p_{\mathrm{L}} = 645.8\,\mathrm{kPa}$ ($0.6458\,\mathrm{MPa}$, absolute), temperature
$T_1 = 24\,^\circ\mathrm{C}$ (expansion-zone local temperature). State: 100\%
saturated liquid (Table~\ref{tab:states}, row 1). Specific enthalpy $h_1 = h_{\mathrm{f,L}} = 84.98\,\mathrm{kJ}/\mathrm{kg}$ (NIST).
...
state 1 and raises pressure from $645.8\,\mathrm{kPa}$ to $685.4\,\mathrm{kPa}$.
No vapour is compressed. Pump work:
\begin{equation}
\label{eq:pump_work}
w_{\mathrm{pump}} = v_{\mathrm{f}}\,(p_{\mathrm{H}} - p_{\mathrm{L}})
= 0.0008261 \times (685.4 - 645.8) \times 10^3 = 32.7\,\mathrm{J}/\mathrm{kg}
= 0.0327\,\mathrm{kJ}/\mathrm{kg}.
\end{equation}
The formula is standard for incompressible liquid (Eq.~\ref{eq:pump_work}). State 2: \textbf{Subcooled liquid}. $p_2 = 685.4\,\mathrm{kPa}$,
$T_2 = 24\,^\circ\mathrm{C}$. Since $p_2 > p_{\mathrm{sat}}(24\,^\circ\mathrm{C})
= 645.8\,\mathrm{kPa}$, the fluid is subcooled and stable (no boiling).
$h_2 = h_1 + w_{\mathrm{pump}} = 84.98 + 0.0327 = 85.01\,\mathrm{kJ}/\mathrm{kg}$. With a
real pump (isentropic efficiency $\eta_{\mathrm{pump}} \approx 0.6$--$0.8$), the
actual pump work may be $0.04$--$0.05\,\mathrm{kJ}/\mathrm{kg}$; $w_{\mathrm{net}}$
remains positive (e.g.\ $0.64$--$0.68\,\mathrm{kJ}/\mathrm{kg}$).
...
$p_3 = 685.4\,\mathrm{kPa}$, $T_3 = 26\,^\circ\mathrm{C}$, so
$h_3 = h_{\mathrm{f,H}} = 87.83\,\mathrm{kJ}/\mathrm{kg}$ (NIST). Thus
$q_{\mathrm{absorb}} = h_3 - h_2 = 87.83 - 85.01 = 2.82\,\mathrm{kJ}/\mathrm{kg}$.
State 3: $h_3 = 87.83\,\mathrm{kJ}/\mathrm{kg}$, retaining
...
($p_{\mathrm{L}} = 645.8\,\mathrm{kPa}$, $T \approx 24\,^\circ\mathrm{C}$). The
...
expansion from state 3 (liquid at 685.4\,kPa, 26\,$^\circ$C) to $p_{\mathrm{L}}$,
...
\end{equation}
For the expander (adiabatic, no heat transfer), the first law gives
$h_3 - h_4 = w_{\mathrm{out}}$, so
$h_4 = h_3 - w_{\mathrm{out}} = 87.83 - 0.72 = 87.11\,\mathrm{kJ}/\mathrm{kg}$.
This value lies in the two-phase range at $p_{\mathrm{L}}$ ($h_{\mathrm{f,L}} < h_4 < h_{\mathrm{f,L}}+h_{\mathrm{fg}}$) and is used in the process 4$\to$1 balance below. The expander is the only work-producing step and the pump is the only work-consuming step. If $\eta_{\mathrm{exp}} = 0.4$, then $w_{\mathrm{out}} = 0.48\,\mathrm{kJ}/\mathrm{kg}$
...
\begin{equation}
\label{eq:w_net}
w_{\mathrm{net}} = w_{\mathrm{out}} - w_{\mathrm{pump}} = 0.72 - 0.0327
= 0.687\,\mathrm{kJ}/\mathrm{kg},
\qquad
\eta = \frac{w_{\mathrm{net}}}{q_{\mathrm{absorb}}} = \frac{0.687}{2.82}
\approx 24.4\%,
\end{equation}
where $\eta$ is the cycle (energy) efficiency. Net work is \textbf{positive}. At 1--2\,$^\circ$C nominal temperature span, the
Carnot efficiency would be about 0.33--0.67\%; this cycle reaches 24.4\% under
...
\begin{itemize}
  ...
  $q_{\mathrm{in}} = h_3 - h_2 = q_{\mathrm{absorb}} = 2.82\,\mathrm{kJ}/\mathrm{kg}$.
  ...
  $w_{\mathrm{net}} = w_{\mathrm{out}} - w_{\mathrm{pump}} = 0.687\,\mathrm{kJ}/\mathrm{kg}$.
  ...
  $q_{\mathrm{out}} = q_{\mathrm{in}} - w_{\mathrm{net}} = 2.82 - 0.687
  = 2.13\,\mathrm{kJ}/\mathrm{kg}$. Process 4$\to$1 (condensation, no shaft work) satisfies the first law per unit mass: $q_{\mathrm{out}} = h_4 - h_1$; with $h_4 = h_3 - w_{\mathrm{out}} = 87.11\,\mathrm{kJ}/\mathrm{kg}$ from the expander step, $h_4 - h_1 = 87.11 - 84.98 = 2.13\,\mathrm{kJ}/\mathrm{kg}$, consistent with the cycle balance.
\end{itemize}
So the cycle (in the design) delivers \textbf{0.687\,kJ useful work per kilogram of fluid per cycle},
from \textbf{2.82\,kJ heat absorbed per kilogram per cycle}; 2.13\,kJ/kg is rejected
...
w_{\mathrm{net}}$ (2.82 - 2.13 = 0.687\,kJ/kg). Last row:
...
\midrule
1$\to$2 (pump) & 0 & 0 & 0.0327 & 0 \\
2$\to$3 (absorb heat) & 2.82 & 0 & 0 & 0 \\
3$\to$4 (expander) & 0 & 0 & 0 & 0.72 \\
4$\to$1 (condensation) & 0 & 2.13 & 0 & 0 \\
\midrule
\textbf{Total (cycle)} & \textbf{2.82} & \textbf{2.13} & \textbf{0.0327} & \textbf{0.72} \\
\midrule
Maintain $\Delta T$ (optional heat pump, kJ/kg) & -- & -- & 0.019--0.044 & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.9, every node/.style={font=\small},
  ...
  \draw[->, thick] (e1) -- (pump.west) node[lbl, pos=0.5, above=1pt] {$W_{\mathrm{in}}{=}0.0327$};
  \draw[->, thick] (e2) -- (hp.west) node[lbl, pos=0.5, above=1pt] {$Q_{\mathrm{in}}{=}2.82$};
  \draw[->, thick] (exp.west) -- (e3) node[lbl, pos=0.5, above=1pt] {$W_{\mathrm{out}}{=}0.72$};
  \draw[->, thick] (ch.west) -- (e4) node[lbl, pos=0.5, below=1pt] {$Q_{\mathrm{out}}{=}2.13$};
  \node[align=center, font=\footnotesize] at (2.5,-3.4) {Net: $w_{\mathrm{net}}=0.72-0.0327=0.687$; optional $W_{\mathrm{HP}}\approx0.019$--0.044\,kJ/kg to maintain $\Delta T$.};
\end{tikzpicture}
\caption{Energy flows between environment and cycle components (per kg,
kJ/kg). Each stage: heat absorbed (2$\to$3 only, $Q_{\mathrm{in}}{=}2.82$), heat rejected (4$\to$1, $Q_{\mathrm{out}}{=}2.13$), work in (1$\to$2 pump, $W_{\mathrm{in}}{=}0.0327$), work out (3$\to$4 expander, $W_{\mathrm{out}}{=}0.72$). Net work $w_{\mathrm{net}}=0.687$. Totals match the first-law balance $2.82 - 2.13 = 0.687$. Energy used to maintain the 1--2\,$^\circ$C difference (optional heat pump): 0.019--0.044\,kJ/kg. This figure is distinct from Fig.~\ref{fig:cycle} (process layout). All values in kJ/kg.}
...
$2.82 - 2.13 = 0.687\,\mathrm{kJ}/\mathrm{kg}$. No energy is unaccounted for.
...
$\dot{Q}_{\mathrm{in}} = \dot{m}\,q_{\mathrm{in}} \approx 2.82\,\dot{m}\,\mathrm{kW}$
and net power output is
$\dot{W}_{\mathrm{net}} = \dot{m}\,w_{\mathrm{net}} \approx 0.687\,\dot{m}\,\mathrm{kW}$.
...
\textbf{Ideal (Carnot) heat pump} moving heat $q_{\mathrm{in}} = 2.82\,\mathrm{kJ}/\mathrm{kg}$
...
= 2.82 \times \frac{2}{299} \approx 0.0189\,\mathrm{kJ}/\mathrm{kg}.
\end{equation}
All temperatures and heat flows in this estimate are specified above (Eq.~\ref{eq:HP_ideal}). The cycle's net output is $w_{\mathrm{net}} = 0.687\,\mathrm{kJ}/\mathrm{kg}$, so
$w_{\mathrm{HP,ideal}} \ll w_{\mathrm{net}}$: only about
$0.0189/0.687 \approx 2.75\%$ of the net output would be needed to run an ideal
heat pump that replenishes the hot side with the heat the cycle absorbs. The
remaining $\approx 97\%$ is \textbf{surplus} (available for external use).
...
0.687\,\mathrm{kJ}/\mathrm{kg}$ is fixed by the cycle parameters. To achieve
...
2.82\,\mathrm{kJ}/\mathrm{kg}$ in process 2$\to$3 from the high-pressure zone at
...
$q_{\mathrm{out}} \approx 2.13\,\mathrm{kJ}/\mathrm{kg}$ to the same
...
\begin{enumerate}
  ...
  \approx 2.82\,\mathrm{kJ}/\mathrm{kg}$).
...
\begin{itemize}
  \item \textbf{Pressure}: 645.8--685.4\,kPa (0.646--0.685\,MPa, absolute) is low; standard refrigeration
...
$w_{\mathrm{net}} = 0.687\,\mathrm{kJ}/\mathrm{kg}$ (Eq.~\ref{eq:w_net}), and rejects
$q_{\mathrm{out}} \approx 2.13\,\mathrm{kJ}/\mathrm{kg}$ to the same
