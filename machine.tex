...
0.9\,\mathrm{kJ}/\mathrm{kg}$ per cycle), delivers net useful work
($w_{\mathrm{net}} = +0.514\,\mathrm{kJ}/\mathrm{kg}$), and uses only standard
...
the first law of thermodynamics. We choose state 3 so that the fluid remains liquid at
...
upper bound for the ideal expansion work in the expressions below (NIST gives slightly higher enthalpy drop for isentropic expansion). With mechanical (isentropic) efficiency
...
\end{equation}
This value is used in the energy balance below; the expander is the only work-producing step and the pump is the only work-consuming step. If $\eta_{\mathrm{exp}} = 0.4$, then $w_{\mathrm{out}} = 0.36\,\mathrm{kJ}/\mathrm{kg}$
...
\end{itemize}
So the cycle (in the design) delivers \textbf{0.514\,kJ useful work per kilogram of fluid per cycle},
from \textbf{0.9\,kJ heat absorbed per kilogram per cycle}; 0.386\,kJ/kg is rejected
...
w_{\mathrm{net}}$ (0.9 - 0.386 = 0.514\,kJ/kg). Last row:
...
\midrule
Maintain $\Delta T$ (optional heat pump, kJ/kg) & -- & -- & 0.006--0.03 & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.9, every node/.style={font=\small},
  ...
  \draw[->, thick] (e2) -- (hp.west) node[lbl, pos=0.5, above=1pt] {$Q_{\mathrm{in}}{=}0.9$};
  ...
  \node[align=center, font=\footnotesize] at (2.5,-3.4) {Net: $w_{\mathrm{net}}=0.54-0.026=0.514$; optional $W_{\mathrm{HP}}\approx0.006$--0.03\,kJ/kg to maintain $\Delta T$.};
\end{tikzpicture}
\caption{Energy flows between environment and cycle components (per kg,
kJ/kg). Each stage: heat absorbed (2$\to$3 only, $Q_{\mathrm{in}}{=}0.9$), heat rejected (4$\to$1, $Q_{\mathrm{out}}{=}0.386$), work in (1$\to$2 pump, $W_{\mathrm{in}}{=}0.026$), work out (3$\to$4 expander, $W_{\mathrm{out}}{=}0.54$). Net work $w_{\mathrm{net}}=0.514$. Totals match the first-law balance $0.9 - 0.386 = 0.514$. Energy used to maintain the 1--2\,$^\circ$C difference (optional heat pump): 0.006--0.03\,kJ/kg. This figure is distinct from Fig.~\ref{fig:cycle} (process layout). All values in kJ/kg.}
...
\end{equation}
All temperatures and heat flows in this estimate are specified above (Eq.~\ref{eq:HP_ideal}). The cycle's net output is $w_{\mathrm{net}} = 0.514\,\mathrm{kJ}/\mathrm{kg}$, so
...
efficiency quoted in this paper (57\% for the R134a four-state cycle) is for the
...
$w_{\mathrm{net}} = +0.514\,\mathrm{kJ}/\mathrm{kg}$ (Eq.~\ref{eq:w_net}), and rejects
$q_{\mathrm{out}} \approx 0.386\,\mathrm{kJ}/\mathrm{kg}$ to the same
...
one reservoir is achieved in the design; the cycle efficiency ($\approx 57\%$, Eq.~\ref{eq:w_net}) is
