...
fields, etc.). Here and throughout, $\lambda$ is understood in a broad sense: all
forces, interactions, rules, couplings, coordination patterns, and driving
protocols that shape the system's dynamics are treated as constraints in this
sense. In particular, constraints (including driving) may be static or dynamic
(time-dependent); the definitions and conclusions apply whenever the stated
axioms/assumptions hold for the corresponding constrained dynamics and the
invariant (or long-time) measures they induce.
...
In the same spirit, crystal semiconductor diodes provide a simple asymmetric-constraint
platform that can satisfy the prerequisite for directional rectification of thermal
motion at the circuit level. Our preliminary tests in non-specialized laboratory
conditions indicate a stable polarity of the generated potential difference;
when diodes are connected in series with the same orientation, the potential
difference increases, while opposite orientation leads to mutual cancellation.
...
The arguments and proofs in this paper are formulated entirely within the
framework of classical physics (classical mechanics and classical statistical
mechanics). In other areas of physics, whenever analogous assumptions about
microscopic reversibility, coarse-grained entropy, and constraint-induced
reshaping of the invariant measure are satisfied, one may expect results that
are equivalent or closely analogous in spirit.
